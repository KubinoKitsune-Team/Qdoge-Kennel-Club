...
const AssetsTransfer: React.FC<{ entity: string }> = ({ entity }) => {
    ...
    useEffect(() => {
        const fetchAssetTransfers = async () => {
            const res = await fetchEntityTransfers(entity);
            console.log(res);
                            ...
                            <TableBody className="divide-y divide-border/40 text-muted-foreground text-xs">
                                {assetTransfers.map((assetTransfer, index) => (
                                    <TableRow key={index}>
                                        ...
                                        <TableCell>{assetTransfer.tick}</TableCell>
                                        <TableCell className="max-w-[150px] truncate">
                                            <Link to={`${EXPLORER_URL}/network/tx/${assetTransfer.hash}`} target="_blank" className="text-blue-500 hover:underline">
                                                {assetTransfer.hash.slice(0, 5)}...{assetTransfer.hash.slice(-5)}
                                            </Link>
                                        </TableCell>
                                        <TableCell>{assetTransfer.source.slice(0,5)}...{assetTransfer.source.slice(-5)}</TableCell>
                                        <TableCell>{assetTransfer.extraData.newOwner.slice(0,5)}...{assetTransfer.extraData.newOwner.slice(-5)}</TableCell>

...
import { fetchEpochRangeTransfers, type EpochTransfer } from "@/services/backend.service";
...
const EpochRangeTransfers: React.FC<{ startEpoch: number; endEpoch: number; searchTerm?: string; connectedWallet?: string | null }> = ({
  startEpoch,
  endEpoch,
  searchTerm = "",
  ...
  useEffect(() => {
    setIsLoading(true);
    setError(null);
    fetchEpochRangeTransfers(startEpoch, endEpoch)
      .then(setTransfers)
  ...
  const filteredTransfers = useMemo(() => {
    ...
    return transfers.filter((t) => t.source.toLowerCase().includes(term) || t.destination.toLowerCase().includes(term));
  ...
  const { senders, receivers, totals } = useMemo(() => {
    ...
    return {
      ...
      totals: [...totalMap.values()]
        .map((t) => ({ ...t, net: t.received - t.sent, totalCount: t.sentCount + t.receivedCount }))
  ...
  return (
    <div className="flex flex-col w-full h-full gap-3">
      <div className="flex items-center justify-between">
        <span className="text-sm md:text-base font-semibold">Monthly Transfers ({startEpoch}~{endEpoch})</span>
...
export default EpochRangeTransfers;
